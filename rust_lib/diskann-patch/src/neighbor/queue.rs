...
impl<I: NeighborPriorityQueueIdType> NeighborPriorityQueue<I> {
    ...
    pub fn insert(&mut self, nbr: Neighbor<I>) {
        ...
        // Clamp insert_idx to new size after truncation.
        // get_lower_bound() can return old self.size when all elements compare
        // less-than via SIMD, but the scalar guard prevented early-return.
        // After truncation reduces vec length by 1, insert_idx == old_size
        // would panic on Vec::insert. Clamping to self.size appends at end
        // which preserves sort order (element has largest distance).
        let insert_idx = insert_idx.min(self.size);
